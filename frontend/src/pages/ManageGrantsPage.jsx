...
import { PlusCircle, Edit, Trash2, User, Eye, AlertCircle } from 'lucide-react';
...
export default function ManageGrantsPage() {
    ...
    // Condition to check if a Grant Maker is logged in but not part of an organization
    const isUnaffiliatedGrantMaker = isGrantMaker && !hasOrganization;

    useEffect(() => {
        const fetchGrants = async () => {
            if (!user?.token || isUnaffiliatedGrantMaker) return;
        ...
        fetchGrants();
    }, [user, isSuperAdmin, isUnaffiliatedGrantMaker]);
    ...
    // Display this message if a Grant Maker is not part of an organization
    if (isUnaffiliatedGrantMaker) {
         return (
            <div className="text-center py-20 bg-white dark:bg-gray-800 rounded-xl shadow-md">
                ...
                <p className="mt-2 text-gray-600 dark:text-gray-300">As a Grant Maker, you must be part of an organization to manage grants. Please join an existing one or create a new one.</p>
