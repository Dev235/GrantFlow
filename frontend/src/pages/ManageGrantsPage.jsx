...
export default function ManageGrantsPage() {
    ...
    // Get status from URL params for filtering
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold text-gray-800">{isSuperAdmin ? 'Manage All Grants' : 'Manage Your Grants'}</h1>
                {!isSuperAdmin && (
            ...
            <div className="bg-white p-4 rounded-xl shadow-md">
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {['All', 'Draft', 'Active', 'Inactive'].map(status => (
                        <button key={status} onClick={() => setStatusFilter(status)} className={`px-3 py-1 text-sm rounded-full ${statusFilter === status ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'}`}>
                            {status}
                        </button>
                    ))}
                </div>
            ...
            {filteredGrants.length > 0 ? (
                        ...
                        {filteredGrants.map(grant => (
                            <li key={grant._id} className="p-4 flex flex-col md:flex-row justify-between items-start md:items-center hover:bg-gray-50">
                                ...
                                <div className="flex items-center gap-2">
                                    <select value={grant.status} onChange={(e) => handleStatusChange(grant._id, e.target.value)} className="text-sm rounded-md border-gray-300 shadow-sm">
                                        <option value="Draft">Draft</option>
                                        <option value="Active">Active</option>
                                        <option value="Inactive">Inactive</option>
                                    </select>
                                    <button onClick={() => navigate(`/manage/applications/${grant._id}`)} className="p-2 text-gray-600 rounded-md hover:bg-gray-200" title="View Applications"><Eye size={18}/></button>
                                    {!isSuperAdmin && <button onClick={() => navigate(`/manage/grants/edit/${grant._id}`)} className="p-2 text-indigo-600 rounded-md hover:bg-indigo-100" title="Edit Grant"><Edit size={18}/></button>}
                                    <button onClick={() => handleDeleteClick(grant)} className="p-2 text-red-500 rounded-md hover:bg-red-100" title="Delete Grant"><Trash2 size={18}/></button>
                                </div>
                            </li>
                        ))}
                    </ul>
                ...
                <div className="text-center py-10 bg-white rounded-xl shadow-md">
                    ...
                    <p className="text-gray-500 mt-2">No grants match the "{statusFilter}" filter.</p>
                </div>
            )}
