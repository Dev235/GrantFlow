...
export default function ManageGrantsPage() {
    ...
    const [grantsPerPage] = useState(5);
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {['All', 'Draft', 'Active', 'Inactive'].map(status => (
                        <button
                            ...
                            className={`px-3 py-1 text-sm rounded-full ${statusFilter === status ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300 dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500'}`}
                        >
                            {status}
                        </button>
                    ))}
            ...
            {currentGrants.length > 0 ? (
                        ...
                        {currentGrants.map(grant => (
                            <li key={grant._id} className="p-4 flex flex-col md:flex-row justify-between items-start md:items-center hover:bg-gray-50 dark:hover:bg-gray-700">
                                    ...
                                    <div className="flex items-center flex-wrap gap-x-4 gap-y-1 text-sm text-gray-500 dark:text-gray-400 mt-1">
                                        ...
                                        {isSuperAdmin && grant.grantMaker && (
                                            <span className="flex items-center gap-1">
                                                <User size={14}/> {grant.grantMaker.name}
                                            </span>
                                        )}
                                    </div>
                                </div>
                                <div className="flex items-center gap-2">
                                    <select
                                        value={grant.status}
                                        onChange={(e) => handleStatusChange(grant._id, e.target.value)}
                                        className="text-sm rounded-md border-gray-300 dark:border-gray-600 shadow-sm bg-white dark:bg-gray-700"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="p-2 text-gray-600 rounded-md hover:bg-gray-200 dark:text-gray-300 dark:hover:bg-gray-600"
                                    ...
                                    {!isSuperAdmin && (
                                        <button
                                            onClick={() => navigate(`/manage/grants/edit/${grant._id}`)}
                ...
                <div className="text-center py-10 bg-white dark:bg-gray-800 rounded-xl shadow-md">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mt-2">
                        No grants match the "{statusFilter}" filter.
                    </p>
                </div>
            )}
            ...
            >
                Are you sure you want to delete the grant "<strong>{grantToDelete?.title}</strong>"? 
                This will also remove all associated applications and cannot be undone.
            </ConfirmationModal>
        </div>
    );
}
