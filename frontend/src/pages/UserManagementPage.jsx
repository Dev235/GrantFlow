...
const ViewUserModal = ({ user, isOpen, onClose, onVerify }) => {
    ...
    const handleResetPassword = async () => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/users/${user._id}/reset-password`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${currentUser.token}`,
...
export default function UserManagementPage() {
    ...
    const confirmDelete = async () => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/users/${userToProcess._id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${currentUser.token}` },
            });
            if (!response.ok) {
    ...
    const handleVerifyUser = async (userToVerify) => {
        try {
            await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/users/${userToVerify._id}/verify`, {
                method: 'PUT',
                headers: { 'Authorization': `Bearer ${currentUser.token}` },
            });
